...
const Card = ({ stage, cardNumber, cardholderName, validThru, securityCode }) => {
  ...
  const displaySecurityCode = securityCode.value + 'XXX'.slice(securityCode.value.length)
  const cardNumArr = [ 
    ...
    <div className='card-container'>
      <div className={`card ${stage === 'security-code' ? 'flipped' : ''}`}>
        ...
        <div className='face back'>
          <div className={`security-code ${stage === 'security-code' ? 'active' : 'inactive'}`}>
            <div>{displaySecurityCode.split('').map((num,i) => {
              if (num !== 'X') return <span className='num' key={i}>{num}</span>
              else return <span key={i}>{num}</span>
            })}</div>
          </div>
          <FaReact />          
        </div>
